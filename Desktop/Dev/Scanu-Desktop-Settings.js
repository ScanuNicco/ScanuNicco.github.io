...
function drop(event) { 
    ...
    var dragSettings = document.getElementById('settings');
    dragSettings.style.left = (event.clientX + parseInt(offset[0],10)) + 'px';
    dragSettings.style.top = (event.clientY + parseInt(offset[1],10)) + 'px';
    event.preventDefault();
    return false;
} 
var dragSettings = document.getElementById('settings'); 
dragSettings.addEventListener('dragstart',drag_start,false); 
